...
//        log.error("{}级别,当前时间{}:","error",new Date());
    ...
    public R deleteTeacherById(@PathVariable String id) {
        ...
        teacherService.removeAvatarById(module,id);
    ...
    public R pageQuery(@PathVariable Integer pageNum, @PathVariable Integer pageSize, QueryTeacherBean queryTeacherBean){
        Page<Teacher> page = new Page<>();
        page.setCurrent(pageNum).setSize(pageSize);

        Page<Teacher> pageModel = teacherService.queryPageConditions(page,queryTeacherBean);
